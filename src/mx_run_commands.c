...
static int check_bins(char **commands, char **env, t_ush data, int run_mode) {
    int	i = -1;
	char *bin_path;
	char **path;
	struct stat	f;

    if (run_mode || run_mode == 2)
        path = mx_strsplit(mx_strjoin(mx_get_env_var("PATH", env),
                           mx_strjoin(":", mx_get_env_var("PWD",env))), ':');
    else
        path = mx_strsplit(mx_get_env_var("PATH", data.var), ':');
    while (path && path[++i]) {
...
int mx_run_command(char **commands, t_ush data, char ***env, int run_mode) {
    ...
    if (run_mode == 0 || run_mode == 2) {
        if (check_bins(commands, *env, data, run_mode))
            return 0;
    } 
    else if (run_mode == 1) {
        if ((is_builtin = check_builtins(commands, data, env)) == 1 
            || check_bins(commands, *env, data, run_mode) )
            return 0;
    }
